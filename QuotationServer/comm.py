...
def get_finance_from_date(finance, s, k, date: datetime.datetime):
    for p in finance[s]:
        if date.date() >= datetime.datetime.strptime(p['statDate'], '%Y%m%d %H:%M:%S').date():
            return p[k]
...
def tol_format(code, tol):
    """
    返回可输入数量，None或int
    """
    result = None
    if is_kzz(code) == 1:
        tol = int(tol // Decimal('10') * Decimal('10'))
        if tol < 10:
            return result
    elif is_kzz(code) == 2:
        tol = int(tol // Decimal('10') * Decimal('10') // Decimal('10'))
        if tol < 1:
            return result
    else:
        tol = int(tol // Decimal('100') * Decimal('100'))
        if tol < 100:
            return result
    return tol

def buy_tol_from_money(code, balance, price, cangwei: Decimal):
    """
    根据金额和仓位计算买入数量 None或int
    :param mbalance:
    :param price:
    :param cangwei:
    :return:
    ...
    jy_m -= fee
    tol = jy_m // price
    return tol_format(code, tol)

def sell_money_from_tol(code, price, tol, f=True):
    """
    根据数量计算卖出后的金额
    :param code:
    :param price:
